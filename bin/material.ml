...
type material_scatter = Ray.ray -> Vec3.vec3 -> Vec3.vec3 -> bool ->(Vec3.vec3 * Ray.ray) option
...
let dialetic_scatter ~refraction_index ray point normal front_face =
    ...
    let direction = if cannot_refract || reflectance cos_theta ri > random_float_range 0. 1. then
        Vec3.reflect unit_direction normal
    else
        Vec3.refract unit_direction normal ri in
    Some (Vec3.make 1. 1. 1., Ray.{origin = point; direction = direction})
